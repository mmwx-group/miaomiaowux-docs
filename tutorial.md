...
脚本会先选择本机安装或 Docker Compose，再选择 SQLite 或 PostgreSQL 18。小型部署可使用 SQLite；多服务器或高并发写入建议 PostgreSQL。选择 PostgreSQL 后数据库安装、账号创建和主控连接配置均自动完成。
...
> [!NOTE]
> Docker 镜像已内置 Nginx。使用 `network_mode: host` 启动且宿主机 80/443 未被占用时，也可以直接将证书部署到主控，无需额外安装本机 Agent。
