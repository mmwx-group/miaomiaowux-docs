    ...
    <XDocLayout title={t("update.title")} description={t("update.description")}>
      ...
      <section className="mb-10">
        <h2 className="text-2xl font-bold mb-4">
          {t("update.docker.heading")}
        </h2>
        <p className="text-muted-foreground mb-4">{t("update.docker.text")}</p>
        <h3 className="font-semibold mb-3">
          {t("update.docker.composeHeading")}
        </h3>
        <Card className="mb-6">
          <CardContent className="pt-6">
            <div className="bg-muted rounded-lg p-4 font-mono text-sm overflow-x-auto">
              <pre>{`# 在 docker-compose.yml 所在目录执行
docker compose pull
docker compose up -d`}</pre>
            </div>
          </CardContent>
        </Card>
        <h3 className="font-semibold mb-3">{t("update.docker.runHeading")}</h3>
        <Card>
          <CardContent className="pt-6">
            <div className="bg-muted rounded-lg p-4 font-mono text-sm overflow-x-auto">
              <pre>{`# 拉取最新镜像
...
docker run -d \\
  ...
  ghcr.io/iluobei/miaomiaowux:latest`}</pre>
        ...
        <p className="text-destructive mt-3 text-sm">
          {t("update.docker.mountWarning")}
        </p>
      </section>

      <section className="mb-10">
        <h2 className="text-2xl font-bold mb-4">
          {t("update.binary.heading")}
        </h2>
        <Card>
          <CardContent className="pt-6">
            <p className="text-muted-foreground mb-4">
              {t("update.binary.text")}
            </p>
            <div className="bg-muted rounded-lg p-4 font-mono text-sm overflow-x-auto">
              <pre>{`# 停止服务
