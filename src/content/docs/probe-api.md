...
探针在「系统设置 → 探针」选项卡开启：可把站点伪装为公开监控页，配置标题、Logo、隐蔽登录、展示的服务器范围，以及 CPU / 内存 / 硬盘 / Ping 等 Agent 采集项；服务器的地区、服务商、续费价格等展示信息在「服务管理 → 编辑服务器 → 探针展示信息」里填写。

![系统设置探针选项卡截图](../../assets/screenshots/settings-probe.webp)
