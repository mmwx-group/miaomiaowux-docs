...
远程服务器的 Agent 部署无需手动下载或配置。在主控端添加服务器后，系统会自动生成一键安装命令，在远程服务器上执行即可完成全部部署。安装成功后，服务管理页的服务器卡片会变为在线：

![Agent 在线的服务器卡片截图](../../assets/screenshots/servers-card-online.webp)

Agent 连上后：在线、锁图标（加密通信）、WS 连接、Xray 运行中、Agent 版本

:::caution[重要约束：每台服务器必须使用独立的 agent_token]
...
1. 登录主控端管理面板
2. 进入「服务管理」页面
3. 点击「添加服务器」，填写服务器名称和服务器地址，选择 Xray 模式
4. 点「生成 Token」，系统自动生成该服务器的安装命令

![添加远程服务器对话框截图](../../assets/screenshots/add-server-dialog-filled.webp)

添加远程服务器：填名称、地址，选外置 / 内联 Xray

![生成 Token 后的安装命令截图](../../assets/screenshots/add-server-result.webp)

生成 Token 后：一键安装命令与 Docker 环境变量（含 MMWX_MASTER_PUBLIC_KEY）
...
除了一键脚本(裸机 + systemd),也可以直接拉镜像。镜像内置 embedded xray-core + nginx + 全套依赖,拉起即用,不需要在容器内现装任何东西。「生成 Token」对话框下半部分就是 Docker 部署所需的环境变量,直接复制进 compose 的 environment 即可。

:::caution[必须用 host 网络模式启动]
...
docker run -d \
  ...
  -e MMWX_MASTER_PUBLIC_KEY=<主控身份公钥> \
  -e TZ=Asia/Shanghai \
...
注意 `./config` **不会被自动写入 config.yaml** —— Docker 部署的配置来源就是上面那组环境变量,容器不会把它们落成配置文件。挂出来是空目录属于正常,不代表没配置成功;只有你想手写 config.yaml 并用 `-c /etc/mmw-agent/config.yaml` 指过去时才需要往里放东西。

`MMWX_MASTER_PUBLIC_KEY` 是主控身份公钥。一键脚本会自动把它写进 config.yaml,Docker 部署不走脚本,所以要手动填 —— 主控开了「强制加密」而这一项缺失时,agent 不会发起密钥交换,会一直刷 `authentication failed: Encryption required` 连不上。去哪拿:主控面板 → 服务管理 → 该服务器 → 详情 →「Docker 部署环境变量」,整段复制即可。

`TZ` 与 `/etc/localtime` 决定容器内的时区。两者都不设时容器按 UTC 跑,日志时间会比本地差几个小时。挂 `/etc/localtime` 是跟宿主保持一致;想让容器用另一个时区就只设 `TZ`(两者都在时以 `TZ` 为准)。

token 的环境变量名是 `MMWX_TOKEN`。早期文档写成 `MMWX_MASTER_TOKEN`,agent 不认那个名字,照抄的话容器起来只会一直刷 `authentication failed: Token required`。新版 agent 已同时兼容旧名,但新部署请用 `MMWX_TOKEN`。
...
主控发布新版本后，服务管理页的卡片版本徽标会带红点，点它或顶部「一键升级 Agent」即可远程升级，无需 SSH；卡片「Agent」菜单里还有「卸载 Agent」。

![Agent 菜单截图](../../assets/screenshots/servers-agent-menu.webp)

Agent 菜单：同步节点 / 同步节点地址 / 配置历史 / 下发默认配置 / 网站管理 / 升级 Agent / 卸载 Agent

如果 Agent 出现问题，可以重新打开「编辑服务器」拿到安装命令再次执行。脚本会自动停止旧服务、覆盖安装并重新启动。
