...
Certimate's webhook deployer substitutes `${CERTIMATE_DEPLOYER_*}` template variables with the actual cert content (raw PEM, no base64). MiaomiaowuX auto-detects the -----BEGIN prefix and accepts both raw PEM and the legacy base64 upload paths.
