...
import { DocDemo } from "@/components/docs/islands/add-server";

## Overview

Agent deployment on remote servers requires no manual download or configuration. After adding a server on the master, the system automatically generates a one-click install command that can be run on the remote server to complete the entire deployment. Once installed, the server card on the Servers page turns online:

![Online server card screenshot](../../../assets/screenshots/servers-card-online.webp)

After the Agent connects: online, lock icon (encrypted channel), WS connection, Xray running, Agent version

:::caution[Important: each server must use a unique agent_token]
...
Add remote server: name, address, external / embedded Xray

![Install command after token generation screenshot](../../../assets/screenshots/add-server-result.webp)

After "Generate Token": the one-click install command and the Docker environment variables (including MMWX_MASTER_PUBLIC_KEY)
...
The lower half of the "Generate Token" dialog lists the Docker environment variables; copy them into the compose `environment` block.

:::caution[Must run with host network mode]
...
docker run -d \
  ...
  -e MMWX_TOKEN=<token generated when adding the server on the master> \
  -e MMWX_MASTER_PUBLIC_KEY=<master identity public key> \
  -e TZ=Asia/Shanghai \
  ...
  ghcr.io/mmwx-group/mmwx-agent:latest
```
...
Note that `./config` **is never populated with a config.yaml** — for Docker deployments the environment variables above *are* the configuration; the container does not write them out to a file. An empty directory here is normal and does not mean the setup failed. You only put something in it if you want to hand-write a config.yaml and point the agent at it with `-c /etc/mmw-agent/config.yaml`.

`MMWX_MASTER_PUBLIC_KEY` is the master identity public key. The one-click script writes it into config.yaml automatically; Docker deployments do not run that script, so you must set it by hand. If the master has force-encryption enabled and this is missing, the agent never starts a key exchange and keeps logging `authentication failed: Encryption required`. Where to get it: master panel -> Servers -> the server -> details -> "Docker deployment environment variables", and copy the whole block.

`TZ` and `/etc/localtime` control the timezone inside the container. With neither set the container runs on UTC and log timestamps are hours off. Mounting `/etc/localtime` keeps it in sync with the host; set only `TZ` if you want a different timezone (when both are present, `TZ` wins).

The token variable is `MMWX_TOKEN`. Older docs said `MMWX_MASTER_TOKEN`, which the agent does not read — copying that leaves the container looping on `authentication failed: Token required`. Recent agents accept the old name as well, but use `MMWX_TOKEN` for new deployments.
...
services:
  mmw-agent:
    ...
    environment:
      ...
      - MMWX_TOKEN=<token generated when adding the server on the master>
      - MMWX_MASTER_PUBLIC_KEY=<master identity public key>
      - TZ=Asia/Shanghai
    volumes:
...
When a new release is out, the version badge on the server card shows a red dot; click it or "Upgrade Agents" at the top to upgrade remotely without SSH. The card's "Agent" menu also offers "Uninstall Agent".
...
If the Agent has issues, open "Edit server" to get the install command again and re-run it. The script automatically stops the old service, reinstalls, and restarts.
