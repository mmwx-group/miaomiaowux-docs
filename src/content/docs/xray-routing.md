...
![路由管理面板截图](../../assets/screenshots/servers-xray-manage-routing.webp)

Xray 管理 → 路由管理：规则列表可拖拽排序，顶部「快捷添加 / 自定义规则 / 负载均衡」
...
| 入口     | 位置                                   | 说明                                 |
| -------- | -------------------------------------- | ------------------------------------ |
| 节点路由 | 节点管理页 → 节点行内的「节点路由」按钮 | 查看和管理单个节点（入站）的路由规则 |
| 路由面板 | 服务管理 → Xray配置 → 路由管理选项卡    | 管理服务器全部路由规则，支持拖拽排序 |
...
![节点路由对话框截图](../../assets/screenshots/nodes-row-routing.webp)

节点路由：上方是该入站的专属规则，下方是对所有入站生效的全局规则与默认出站
...
在服务管理页面点服务器卡片的「Xray配置」，切到「路由管理」选项卡，可以管理远程服务器的全部路由规则。截图里可以看到系统自动加入的几类规则：

- **Reality 防盗**：开启「防止 Reality 被盗用」的入站会自动生成两条规则——放行 serverNames 域名到 direct，其余经防盗 tunnel 进来的流量 block
- **禁止 BT / 禁止访问大陆 IP / geoip:private → block**：默认的安全规则，可删除
...
| 防止送中        | geosite:google + meta  | warp-v4 | 仅已安装 WARP 的服务器显示 |
...
### 示例：只让某个入站的 Netflix 流量走美国落地

1. 先在节点管理为该节点「创建链式出站」，选美国落地节点，得到一个代理出站（tag 形如 `landing-<入站tag>-<时间戳>`）
2. 打开该节点的「节点路由」→「自定义规则」
3. domain 填 `geosite:netflix`，inboundTag 保持为当前入站，outboundTag 选刚创建的落地出站
4. 保存后 Xray 自动重启；规则出现在「专属路由规则」里，其余流量仍按全局规则与默认出站处理
