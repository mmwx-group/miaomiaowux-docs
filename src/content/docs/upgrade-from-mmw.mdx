...
用管理员账号登录 mmwx 后，点右上角头像菜单 → 「从妙妙屋迁移」，即可进入 5 步向导。

![头像菜单截图](../../assets/screenshots/user-menu.webp)

头像菜单：「从妙妙屋迁移」入口只对管理员可见

![迁移向导页面截图](../../assets/screenshots/migrate-from-mmw.webp)

真实主控的迁移向导第一步：迁移保证与前置要求该入口对普通用户不可见，后端 6 个 /api/admin/migrate/\* 接口全部受 RequireAdmin 保护，非管理员调用会返回 403。
